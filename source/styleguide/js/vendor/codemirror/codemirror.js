...
;(function(mod) {
	...
	function CodeMirror(place, options) {
		...
		if ((options.autofocus && !mobile) || cm.hasFocus()) setTimeout(bind(onFocus, this), 20)
		else onBlur(this)
	...
	function setDocumentHeight(cm, measure) {
		...
		cm.display.gutters.style.height =
			Math.max(
				measure.docHeight + cm.display.barHeight + scrollGap(cm),
		...
		if (line.markedSpans)
			for (var i = 0; i < line.markedSpans.length; ++i) {
				var sp = line.markedSpans[i],
					m = sp.marker
				if (
					(sp.from == null || (m.inclusiveLeft ? sp.from <= pos.ch : sp.from < pos.ch)) &&
	...
	function highlightWorker(cm) {
		...
		doc.iter(doc.frontier, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function(
			line
		) {
	...
	function coordsChar(cm, x, y) {
		...
		for (;;) {
			...
			if (
				merged &&
				(found.ch > mergedPos.from.ch || (found.ch == mergedPos.from.ch && found.xRel > 0))
			)
	...
	function endOperation_finish(op) {
		...
		if (
			...
			doc.scrollLeft = Math.max(
				0,
				Math.min(display.scroller.scrollWidth - display.scroller.clientWidth, op.scrollLeft)
			)
	...
	function leftButtonSelect(cm, e, start, type, addNew) {
		...
		if (!addNew) {
			...
			setSelection(
				doc,
				normalizeSelection(ranges.slice(0, ourIndex).concat(ranges.slice(ourIndex + 1)), 0),
		...
		function extendTo(pos) {
			...
			if (type == 'rect') {
				...
				setSelection(
					doc,
					normalizeSelection(startSel.ranges.slice(0, ourIndex).concat(ranges), ourIndex),
	...
	function addToScrollPos(cm, left, top) {
		...
		if (left != null)
			cm.curOp.scrollLeft =
				(cm.curOp.scrollLeft == null ? cm.doc.scrollLeft : cm.curOp.scrollLeft) + left
		if (top != null)
			cm.curOp.scrollTop =
	...
	option('specialChars', /[\t\u0000-\u0019\u00ad\u200b-\u200f\u2028\u2029\ufeff]/g, function(
		cm,
		val,
		old
	) {
		...
		return function(str) {
			...
			for (var i = 0; i < len; ++i) {
				...
				if (type == ',') types[i] = 'N'
					...
					var replace =
						(i && types[i - 1] == '!') || (end < len && types[end] == '1') ? '1' : 'N'
